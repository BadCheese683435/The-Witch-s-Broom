...
public class BroomEntity extends BoatEntity {
    ...
    protected void updatePassengerPosition(Entity passenger, PositionUpdater positionUpdater) {
        ...
        // 应用扫帚特有的位置偏移
        double newY = passenger.getY() + 0.6 + floatingValue;
        positionUpdater.accept(passenger, passenger.getX(), newY, passenger.getZ());
    }
    ...
    private void clientTick(){
        ...
        if (passenger != null){
            if ((!this.hasPassengers()) && passenger != null && this.getWorld().isClient() && MinecraftClient.getInstance().player.getId() == passenger.getId()){
...
//                if (passenger != null && ((PlayerEntity)passenger).getEquippedStack(EquipmentSlot.HEAD).getItem() instanceof BaseArmor){
                if (passenger != null) {
                      ...
                      boolean hasArmorInHand = false;
                      boolean hasArmorInArmorSlots = false;
                      
                      // 检查手中是否有护甲
                      for (ItemStack stack : player.getHandItems()) {
                          if (stack.getItem() instanceof BaseArmor) {
                              hasArmorInHand = true;
                              break;
                          }
                      }
                      ...
                      for (ItemStack stack : player.getArmorItems()) {
                          if (stack.getItem() instanceof BaseArmor) {
                              hasArmorInArmorSlots = true;
                              break;
                          }
                      }
                      
                      if (hasArmorInHand || hasArmorInArmorSlots) {
                          maxspeed = 0.6f;
                      } else {
                          maxspeed = 0.3f;
                      }
